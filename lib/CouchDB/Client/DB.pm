...
=head1 SYNOPSIS
	...
	#...
        my $result = $db->somedesigndoc_someviewname(); # Autogenerated method
	$db->delete;
...
=head1 AUTOGENERATED VIEW QUERY METHODS

For convenience, methods to send straightforward queries to design
document views will be auto-generated when the database object is
created. The names of the methods will be design document name
(ignoring the "_design/" part), an underscore and then the view name.
So if we have a design document named "utils", and a view in it called
"maxsize", it could be called as:

   my $result = $db->utils_maxsize( group => 1 );

Behind the scenes, this calls the C<queryView> method in the
L<CouchDB::Client::DesignDoc> module with the name "filled in". This
means that any arguments to the autogenerated method will be used as
the C<%ARGS> to C<queryView>. In the snippet above, for example, we
include the argument to turn on grouping.

Note that any new views or design documents added after the object is
created will not automatically show up. Only those that are visible in
the database at the time of object creation will have convenience
